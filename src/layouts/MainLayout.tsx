...
export default function MainLayout() {
  ...
  const navClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
      isActive ? 'bg-primary-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-700'
    }`
    ...
    <div className="flex h-screen bg-surface overflow-hidden">
      <aside className="w-56 bg-surface-card border-r border-surface-border flex flex-col">
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          <p className="text-xs text-slate-600 uppercase tracking-wider px-3 pt-1 pb-1">Operação</p>
          <NavLink to="/pdv" className={navClass}>
            <ShoppingCart size={16} /> Frente de Caixa
          </NavLink>
        ...
        <div className="p-3 border-t border-surface-border space-y-2">
          <div className={`text-xs px-2 py-1 rounded text-center font-medium ${
            cashRegister ? 'bg-green-900/50 text-green-400' : 'bg-red-900/50 text-red-400'
          }`}>
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleLogout}
              className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-900/20 rounded transition-colors" title="Sair">
              <LogOut size={14} />
            </button>
          </div>
        </div>
